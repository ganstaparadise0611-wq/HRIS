<?php
// Overtime attachments API endpoint
// Handles upload, list, and delete operations for overtime request attachments
...
// Upload directory
$uploadDir = __DIR__ . '/uploads/overtime-attachments/';
if (!is_dir($uploadDir)) {
    mkdir($uploadDir, 0755, true);
}

// GET endpoint - List attachments for an overtime request
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $ovt_id = $_GET['ovt_id'] ?? null;
    
    if (empty($ovt_id)) {
        http_response_code(400);
        echo json_encode(['ok' => false, 'message' => 'Missing ovt_id parameter']);
        exit;
    }
    
    [$status, $result, $err] = supabase_request(
        'GET',
        "rest/v1/overtime_attachments?ovt_id=eq.{$ovt_id}&select=*&order=uploaded_at.desc"
    );
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $uniqueName = 'ovt_' . $ovt_id . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
    ...
    $insertData = [
        ...
        'file_size' => $fileSize,
        'mime_type' => $mimeType,
        'file_path' => 'uploads/overtime-attachments/' . $uniqueName,
    ];
    
    [$status, $result, $err] = supabase_request(
        'POST',
        'rest/v1/overtime_attachments',
        [$insertData],
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    [$status, $attachmentData, $err] = supabase_request(
        'GET',
        "rest/v1/overtime_attachments?attachment_id=eq.{$attachment_id}&select=file_path"
    );
    ...
    [$status, $result, $err] = supabase_request(
        'DELETE',
        "rest/v1/overtime_attachments?attachment_id=eq.{$attachment_id}",
        null,
        []
    );
